...
from model_fixed2 import GPTConfig, GPT
...
run_id = '1' #overriden, just default value to be seen on wandb
# data
dataset = 'openwebtext'
...
# model, overriden by init_from if needed
n_layer = 12
n_head = 12
n_embd = 768
...
iterations_per_epoch = total_tokens // tokens_per_iter
...
def get_batch(split):
    ...
    if split == 'train':
        ...
        #calculate total tokens of train split
...
model_args = dict(n_layer=n_layer, n_head=n_head, n_embd=n_embd, block_size=block_size,
                  bias=bias, vocab_size=None, dropout=dropout,   attn_mode=attn_mode, local_attn_ctx=local_attn_ctx) # start with model_args from command line
...
model.to(device)
n_model_params = model.get_num_params()
config['n_model_params'] = n_model_params
...
if wandb_log and master_process:
    import wandb
    wandb.init(project=wandb_project, name=wandb_run_name, config=config, resume='allow', id=run_id)
